...
setwd("C:/Filings_Comparison")

drv <- JDBC("oracle.bi.jdbc.AnaJdbcDriver","bijdbc.jar")
conn <- dbConnect(drv, "jdbc:oraclebi://sprdorabiw-hq.buildings.nycnet:9703/", "tmartin", "Chichewa#9")
...
names(Plumbing_Merge)<- c("Filing.Date", "BIS", "DOB.NOW")

## Stacked Bar Graph
x <- list(title = "")
y <- list(title = "")
Plumbing_Merge<- plot_ly(Plumbing_Merge, x = ~Filing.Date, y = ~BIS, type = 'bar', opacity=0.8, name = 'BIS', hoverinfo='text', text = ~paste('Date:', format(Filing.Date, '%b-%y'),'<br>BIS:', BIS, '<br>DOB.NOW:', DOB.NOW))    %>% add_trace(y = ~DOB.NOW, name = 'DOB.NOW') %>%  layout(xaxis = x, yaxis = y, showlegend = TRUE, barmode='stack', legend = list(orientation = 'h'))  %>% config(displayModeBar = F)
htmlwidgets::saveWidget(as_widget(Plumbing_Merge), "Plumbing_Merge.html")
...
names(Fence_Merge)<- c("Filing.Date", "BIS", "DOB.NOW")

## Stacked Bar Graph
x <- list(title = "")
y <- list(title = "")
Fence_Merge<- plot_ly(Fence_Merge, x = ~Filing.Date, y = ~BIS, type = 'bar', opacity=0.8, name = 'BIS', hoverinfo='text', text = ~paste('Date:', format(Filing.Date, '%b-%y'),'<br>BIS:', BIS, '<br>DOB.NOW:', DOB.NOW))    %>% add_trace(y = ~DOB.NOW, name = 'DOB.NOW') %>%  layout(xaxis = x, yaxis = y, showlegend = TRUE, barmode='stack', legend = list(orientation = 'h'))  %>% config(displayModeBar = F)
htmlwidgets::saveWidget(as_widget(Fence_Merge), "Fence_Merge.html")
...
names(Shed_Merge)<- c("Filing.Date", "BIS", "DOB.NOW")

## Stacked Bar Graph
x <- list(title = "")
y <- list(title = "")
Shed_Merge<- plot_ly(Shed_Merge, x = ~Filing.Date, y = ~BIS, type = 'bar', opacity=0.8, name = 'BIS', hoverinfo='text', text = ~paste('Date:', format(Filing.Date, '%b-%y'),'<br>BIS:', BIS, '<br>DOB.NOW:', DOB.NOW))    %>% add_trace(y = ~DOB.NOW, name = 'DOB.NOW') %>%  layout(xaxis = x, yaxis = y, showlegend = TRUE, barmode='stack', legend = list(orientation = 'h'))  %>% config(displayModeBar = F)
htmlwidgets::saveWidget(as_widget(Shed_Merge), "Shed_Merge.html")
...
names(Standpipe_Merge)<- c("Filing.Date", "BIS", "DOB.NOW")

## Stacked Bar Graph
x <- list(title = "")
y <- list(title = "")
Standpipe_Merge<- plot_ly(Standpipe_Merge, x = ~Filing.Date, y = ~BIS, type = 'bar', opacity=0.8, name = 'BIS', hoverinfo='text', text = ~paste('Date:', format(Filing.Date, '%b-%y'),'<br>BIS:', BIS, '<br>DOB.NOW:', DOB.NOW))    %>% add_trace(y = ~DOB.NOW, name = 'DOB.NOW') %>%  layout(xaxis = x, yaxis = y, showlegend = TRUE, barmode='stack', legend = list(orientation = 'h'))  %>% config(displayModeBar = F)
htmlwidgets::saveWidget(as_widget(Standpipe_Merge), "Standpipe_Merge.html")
...
## Perform outer merge
Sprinker_Merge<- merge(Sprinkler_BIS,Sprinkler_DOB_NOW,by="Filing.Date", all=T)
names(Sprinker_Merge)<- c("Filing.Date", "BIS", "DOB.NOW")

## Stacked Bar Graph
x <- list(title = "")
y <- list(title = "")
Sprinker_Merge<- plot_ly(Sprinker_Merge, x = ~Filing.Date, y = ~BIS, type = 'bar', opacity=0.8, name = 'BIS', hoverinfo='text', text = ~paste('Date:', format(Filing.Date, '%b-%y'),'<br>BIS:', BIS, '<br>DOB.NOW:', DOB.NOW))    %>% add_trace(y = ~DOB.NOW, name = 'DOB.NOW') %>%  layout(xaxis = x, yaxis = y, showlegend = TRUE, barmode='stack', legend = list(orientation = 'h'))  %>% config(displayModeBar = F)
htmlwidgets::saveWidget(as_widget(Sprinker_Merge), "Sprinker_Merge.html")
...
names(Scaffold_Merge)<- c("Filing.Date", "BIS", "DOB.NOW")

## Stacked Bar Graph
x <- list(title = "")
y <- list(title = "")
Scaffold_Merge<- plot_ly(Scaffold_Merge, x = ~Filing.Date, y = ~BIS, type = 'bar', opacity=0.8, name = 'BIS', hoverinfo='text', text = ~paste('Date:', format(Filing.Date, '%b-%y'),'<br>BIS:', BIS, '<br>DOB.NOW:', DOB.NOW))    %>% add_trace(y = ~DOB.NOW, name = 'DOB.NOW') %>%  layout(xaxis = x, yaxis = y, showlegend = TRUE, barmode='stack', legend = list(orientation = 'h'))  %>% config(displayModeBar = F)
htmlwidgets::saveWidget(as_widget(Scaffold_Merge), "Scaffold_Merge.html")
...
names(Sign_Merge)<- c("Filing.Date", "BIS", "DOB.NOW")

## Stacked Bar Graph
x <- list(title = "")
y <- list(title = "")
Sign_Merge<- plot_ly(Sign_Merge, x = ~Filing.Date, y = ~BIS, type = 'bar', opacity=0.8, name = 'BIS', hoverinfo='text', text = ~paste('Date:', format(Filing.Date, '%b-%y'),'<br>BIS:', BIS, '<br>DOB.NOW:', DOB.NOW))    %>% add_trace(y = ~DOB.NOW, name = 'DOB.NOW') %>%  layout(xaxis = x, yaxis = y, showlegend = TRUE, barmode='stack', legend = list(orientation = 'h'))  %>% config(displayModeBar = F)
htmlwidgets::saveWidget(as_widget(Sign_Merge), "Sign_Merge.html")
...
names(Antenna_Merge)<- c("Filing.Date", "BIS", "DOB.NOW")

## Stacked Bar Graph
x <- list(title = "")
y <- list(title = "")
Antenna_Merge<- plot_ly(Antenna_Merge, x = ~Filing.Date, y = ~BIS, type = 'bar', opacity=0.8, name = 'BIS', hoverinfo='text', text = ~paste('Date:', format(Filing.Date, '%b-%y'),'<br>BIS:', BIS, '<br>DOB.NOW:', DOB.NOW))    %>% add_trace(y = ~DOB.NOW, name = 'DOB.NOW') %>%  layout(xaxis = x, yaxis = y, showlegend = TRUE, barmode='stack', legend = list(orientation = 'h'))  %>% config(displayModeBar = F)
htmlwidgets::saveWidget(as_widget(Antenna_Merge), "Antenna_Merge.html")



########## CURB CUT
Curbcut_BIS <- subset(mydata, Curb.Cut.Flag == "Y" | OT.Code.Description=="CURB CUT")
...
names(Curbcut_Merge)<- c("Filing.Date", "BIS", "DOB.NOW")

## Stacked Bar Graph
x <- list(title = "")
y <- list(title = "")
Curbcut_Merge<- plot_ly(Curbcut_Merge, x = ~Filing.Date, y = ~BIS, type = 'bar', opacity=0.8, name = 'BIS', hoverinfo='text', text = ~paste('Date:', format(Filing.Date, '%b-%y'),'<br>BIS:', BIS, '<br>DOB.NOW:', DOB.NOW))    %>% add_trace(y = ~DOB.NOW, name = 'DOB.NOW') %>%  layout(xaxis = x, yaxis = y, showlegend = TRUE, barmode='stack', legend = list(orientation = 'h'))  %>% config(displayModeBar = F)
htmlwidgets::saveWidget(as_widget(Curbcut_Merge), "Curbcut_Merge.html")
...
names(Elevators_Merge)<- c("Filing.Date", "BIS", "DOB.NOW")

## Stacked Bar Graph
x <- list(title = "")
y <- list(title = "")
Elevators_Merge<- plot_ly(Elevators_Merge, x = ~Filing.Date, y = ~BIS, type = 'bar', opacity=0.8, name = 'BIS', hoverinfo='text', text = ~paste('Date:', format(Filing.Date, '%b-%y'),'<br>BIS:', BIS, '<br>DOB.NOW:', DOB.NOW))    %>% add_trace(y = ~DOB.NOW, name = 'DOB.NOW') %>%  layout(xaxis = x, yaxis = y, showlegend = TRUE, barmode='stack', legend = list(orientation = 'h'))  %>% config(displayModeBar = F)
htmlwidgets::saveWidget(as_widget(Elevators_Merge), "Elevators_Merge.html")
...
names(Electrical_Merge)<- c("Filing.Date", "BIS", "DOB.NOW")

## Stacked Bar Graph
x <- list(title = "")
y <- list(title = "")
Electrical_Merge<- plot_ly(Electrical_Merge, x = ~Filing.Date, y = ~BIS, type = 'bar', opacity=0.8, name = 'BIS', hoverinfo='text', text = ~paste('Date:', format(Filing.Date, '%b-%y'),'<br>BIS:', BIS, '<br>DOB.NOW:', DOB.NOW))    %>% add_trace(y = ~DOB.NOW, name = 'DOB.NOW') %>%  layout(xaxis = x, yaxis = y, showlegend = TRUE, barmode='stack', legend = list(orientation = 'h'))  %>% config(displayModeBar = F)
htmlwidgets::saveWidget(as_widget(Electrical_Merge), "Electrical_Merge.html")
